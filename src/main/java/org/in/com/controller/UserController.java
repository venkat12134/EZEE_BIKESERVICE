...
public class UserController {
	...
	@ResponseBody
	public UserIO updateUser(@RequestBody UserIO userIO, @PathVariable("authtoken") String authtoken) throws Exception {
    ...
    @ResponseBody
	public List<UserIO> getCustomerHistory(@PathVariable("authtoken") String authtoken) throws Exception {

		AuthDTO authDTO = new AuthDTO();
		authDTO = authService.getUserByToken(authtoken);
		List<UserIO> userIO = new ArrayList<UserIO>();
		List<UserDTO> userDto = userService.getCustomerHistory(authDTO);
		if (authDTO != null) {
	...
	@ResponseBody
	public UserIO getCustomerByCode(@PathVariable String code, @PathVariable("authtoken") String authtoken) throws Exception {
		AuthDTO authDTO = new AuthDTO();
		authDTO = authService.getUserByToken(authtoken);
		...
		UserIO userIo = new UserIO();
		if (authDTO != null) {
			userDto = userService.getCustomerByCode(authDTO, userDto);
			userIo.setCode(userDto.getCode());
